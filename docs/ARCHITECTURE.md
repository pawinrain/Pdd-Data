...
阶段 C 至 D4 使用 Playwright Python 异步 API 连接可信配置中的本机回环 CDP；当前商品整体经营扩展继续遵守同一技术和安全边界。采集按精确白名单监听目标页面响应，并在各适配器要求的位置使用 DOM 独立核对店铺、业务日期、截止时间或页面总数。服务只断开自身 CDP 传输，不关闭用户 Chrome、Context 或标签页。默认情况下目标标签页缺失即拒绝采集；当本地配置显式开启 `collection.auto_open_missing_pages` 时，服务可在已连接的专用 Chrome 中新建标签页并导航到配置内的目标 URL（主机名在配置加载时已按固定拼多多主机白名单校验，绝不来自 MCP 参数）：仍然不启动浏览器、不新建浏览器 Context、不关闭标签页、不执行登录，新开页面同样经过登录/验证码/错误页选择器检查，售后列表仍只经站内侧栏进入而不盲开列表 URL。未知接口的发现只记录脱敏元数据，不读或保存正文；适配器经人工核实前保持 `NOT_VERIFIED`。
